...
<div class="editAccordion provider">
	...
	<h3>OAuth Provider (<span class="short_name"><?php echo $provider['short_name'] ?></span>)</h3>
	<table class="mainTable" border="0" cellspacing="0" cellpadding="0" style="margin: 0;">
		<tr>
			<td><label for="short_name">Provider Short Name</label><div class="subtext">A unique short name for you to reference this OAuth provider.</div></td>
		...
		<tr>
			...
			<td><?php echo $providers->get_act_url('oauth_callback') ?>&amp;provider=<span class="short_name"><?php echo $provider['short_name'] ?></span></td>
		</tr>
		<tr><td colspan="2"><h4>Your Application Settings</h4></td></tr>
		<tr>
			<td><label for="client_id">Consumer/Client ID</label><div class="subtext">The client id for your application. Also referred to as a "key."</div></td>
			<td><input type="text" name="providers[<?php echo $i ?>][client_id]" value="<?php echo $provider['client_id'] ?>" /></td>
		</tr>
		<tr>
			<td><label for="client_secret">Consumer/Client Secret</label><div class="subtext">The client secret associated with your client ID.</div></td>
		...
		<tr>
			<td><label for="authorization_url">Authorization URL</label><div class="subtext">URL provided by your service provider for client authorization requests.</div></td>
			<td><input type="text" name="providers[<?php echo $i ?>][authorization_url]" value="<?php echo $provider['authorization_url'] ?>" /></td>
		</tr>
		<tr>
			<td><label for="access_token_url">Access Token URL</label><div class="subtext">URL provided by your service provider for client access token requests.</div></td>
			<td><input type="text" name="providers[<?php echo $i ?>][access_token_url]" value="<?php echo $provider['access_token_url'] ?>" /></td>
		</tr>
		<tr>
			<td><label for="refresh_access_token_url">Refresh Access Token URL</label><div class="subtext">URL provided by your service provider for client access token refresh requests.</div></td>
